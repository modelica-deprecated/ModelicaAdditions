...
"),
      ...
      Diagram(Polygon(points=[0, 0; -100, -100; 100, -100; 100, 100; -100, 100
          ...
          string="%name",
          style(color=0))),
      Terminal(Polygon(points=[0, 0; -100, -100; 100, -100; 100, 100; -100,
              100; 0, 0], style(fillColor=3))),
      Window(
        x=0.21,
        y=0.18,
        width=0.71,
...
<HTML>
"),
      Icon(Polygon(points=[-100, -100; 0, -100; 100, 0; 0, 100; -100, 100; -
              100, -100])),
...
"),
  ...
  public
    MultiBody.Interfaces.Frame_a frame_a(
      ...
      f=fa,
      t=ta) annotation (extent=[-120, -15; -90, 15]);
    MultiBody.Interfaces.Frame_a frame_b(
      S=Sb,
      r0=r0b,
      v=vb,
      w=wb,
...
"),
  ...
  partial model Interact2 "Superclass of CutJoint, Force and Sensor classes"
    extends MultiBody.Interfaces.TwoNoTreeFrames;
    extends MultiBody.Interfaces.Interact;
    SIunits.Velocity vaux[3];
    annotation (Coordsys(
        extent=[-100, -100; 100, 100],
        grid=[1, 1],
  ...
  partial model ExtForceBase "Superclass of external forces"
    MultiBody.Interfaces.Frame_a frame_b annotation (extent=[120, -15; 90, 15]
      );
    annotation (
      Coordsys(
        extent=[-100, -100; 100, 100],
...
"),
      Icon(Line(points=[-88, 0; -64, 30; -36, 52; -2, 62; 28, 56; 48, 44; 64,
            ...
            color=0,
            thickness=2,
            fillColor=0)), Polygon(points=[86, 0; 66, 58; 38, 28; 86, 0],
            style(
            color=0,
            fillColor=0,
            fillPattern=1))),
      Diagram(Line(points=[-88, 0; -64, 30; -36, 52; -2, 62; 28, 56; 48, 44;
              64, 28; 76, 14; 80, 10], style(
            color=0,
            thickness=2,
            fillColor=0)), Polygon(points=[86, 0; 66, 58; 38, 28; 86, 0],
            style(
...
"),
  ...
  function axisRotation
    ...
    annotation (Coordsys(
        ...
        height=0.62));
  algorithm
    S := if i == 1 then [1, 0, 0; 0, Modelica.Math.cos(angle),
      Modelica.Math.sin(angle); 0, -Modelica.Math.sin(angle), Modelica.Math.cos
      (angle)] else if i == 2 then [Modelica.Math.cos(angle), 0, -
      Modelica.Math.sin(angle); 0, 1, 0; Modelica.Math.sin(angle), 0,
      Modelica.Math.cos(angle)] else [Modelica.Math.cos(angle),
      Modelica.Math.sin(angle), 0; -Modelica.Math.sin(angle), Modelica.Math.cos
      (angle), 0; 0, 0, 1];
